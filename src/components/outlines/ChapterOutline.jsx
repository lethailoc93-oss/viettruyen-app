...
import { Book, Plus, Trash2, Edit3, Save, X, Sparkles, Loader2, PenTool, Copy, Check, ChevronDown, ChevronUp } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

export default function ChapterOutline() {
    const { currentStory, chapterOps, updateStoryContent, addCharacter, settingOps, timelineOps, abilityOps, itemOps, organizationOps, updateCurrentInfo } = useStory();
    ...
    const [writingStatus, setWritingStatus] = useState(''); // Two-step progress message
    ...
    const handleWriteChapter = async (chapter) => {
        ...
        try {
            ...
            console.log('💾 Auto-saved chapter content to database');

            // ═══════════════════════════════════════
            // BƯỚC 3: Tự động quét dữ liệu từ chương
            // ═══════════════════════════════════════
            ...
            try {
                ...
                if (scanResult.characters?.length > 0) {
                ...
                // Thêm bối cảnh mới
                if (scanResult.settings?.length > 0) {
                    scanResult.settings.forEach(setting => {
                        settingOps.add({
                            name: setting.name,
                ...
                // Thêm sự kiện timeline mới
                if (scanResult.timeline?.length > 0) {
                ...
                // Thêm năng lực mới
                if (scanResult.abilities?.length > 0) {
                    scanResult.abilities.forEach(a => {
                        abilityOps.add({ name: a.name, owner: a.owner || '', effect: a.effect || '', limitation: a.limitation || '' });
                    });
                }
                ...
                if (scanResult.items?.length > 0) {
                    scanResult.items.forEach(i => {
                        itemOps.add({ name: i.name, owner: i.owner || '', effect: i.effect || '' });
                    });
                }

                // Thêm tổ chức mới
                if (scanResult.organizations?.length > 0) {
                    scanResult.organizations.forEach(o => {
                        organizationOps.add({ name: o.name, purpose: o.purpose || '' });
                    });
                }

                // Cập nhật #0 Current Info
                if (scanResult.currentState) {
                    updateCurrentInfo(
                        scanResult.currentState.time || currentStory?.currentTime || '',
                        scanResult.currentState.location || currentStory?.currentLocation || ''
                    );
                }
            ...
            // Delay clearing status so user can read the summary
            setTimeout(() => {
                setWritingId(null);
                setWritingStatus('');
            }, 3000);
        ...
        >
            <style>{`
                @media (max-width: 768px) {
                    .chapter-list-item { flex-direction: column !important; align-items: stretch !important; }
                    .chapter-list-actions { width: 100% !important; justify-content: flex-start !important; margin-top: var(--space-sm); align-self: stretch; }
                }
            `}</style>
                            ...
                            <div style={{ padding: 'var(--space-lg)' }}>
                                {editingId === chapter.id ? (
                                    <div>
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-sm)', marginTop: 'var(--space-md)' }}>
                                            <button className="btn-icon" onClick={() => setEditingId(null)} title="Hủy">
                                                <X size={16} />
                                            </button>
                                    ...
                                    <div className="chapter-list-item" style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--space-md)' }}>
                                        <div style={{
                                            minWidth: '36px',
                                            height: '36px',
                                        ...
                                        <div className="chapter-list-actions" style={{ display: 'flex', gap: 'var(--space-xs)', flexShrink: 0, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                                            ...
                                            <button
                                                ...
                                                style={{
                                                    background: writingId === chapter.id
                                                        ? 'var(--color-text-tertiary)'
                                                        : 'var(--gradient-secondary)',
                                                    fontSize: 'var(--font-size-xs)',
                                                    padding: '4px 10px'
                                                }}
                                                title={!isKeySet ? 'Cần nhập API key để viết chương' : 'AI viết nội dung chương (2 bước: dàn ý → viết)'}
                                            >
                                            ...
                                            <button className="btn-icon" onClick={() => handleEdit(chapter)} title="Sửa">
                                                <Edit3 size={16} />
                                            </button>
                                        ...
                                        <div style={{
                                            ...
                                            <div style={{
                                                ...
                                                <div style={{ display: 'flex', gap: 'var(--space-xs)' }}>
                                                    <button
                                                        className="btn btn-secondary btn-small"
                                                        onClick={() => handleCopyContent(chapter.id)}
                                                    ...
                                                    <button
                                                        className="btn btn-primary btn-small"
                                                        onClick={() => handleAppendToStory(chapter.id)}
                                                        style={{ fontSize: 'var(--font-size-xs)', padding: '3px 8px' }}
                                                    >
